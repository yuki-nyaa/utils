...
namespace yuki{
    ...
    struct multiset : private std::multiset<Key,Compare,Allocator> {
        ...
        public:
            ...
            multiset(InputIterator first, InputIterator last, const Compare& comp = Compare(), const Equal& eq = Equal(), const Allocator& alloc = Allocator()) : Base_(first,last,comp,alloc),key_compare_(comp),key_equal_(eq),alloc_(alloc),p_has_equiv_(true),p_has_equal_(true) {}
            ...
            multiset(std::initializer_list<key_type> il, const Compare& comp = Compare(), const Equal& eq = Equal(), const Allocator& alloc = Allocator()) : Base_(il,comp,alloc), key_compare_(comp),key_equal_(eq),alloc_(alloc),p_has_equiv_(true),p_has_equal_(true) {}
            ...
            multiset& operator=(std::initializer_list<key_type> il){Base_::operator=(il); p_has_equiv_=true; p_has_equal_=true; return *this;}
            ...
            void clear() noexcept {flags_false_();Base_::clear();}
            ...
            iterator first_greater(const Key& key) {return Base_::upper_bound(key);}
            const_iterator first_greater(const Key& key) const {return Base_::upper_bound(key);}
            template<typename K> requires requires{typename Compare::is_transparent;}
            iterator first_greater(const K& key) {return Base_::upper_bound(key);}
            template<typename K> requires requires{typename Compare::is_transparent;}
            const_iterator first_greater(const K& key) const {return Base_::upper_bound(key);}
            ...
            iterator first_equiv_greater(const Key& key) {return Base_::lower_bound(key);}
            const_iterator first_equiv_greater(const Key& key) const {return Base_::lower_bound(key);}
            template<typename K> requires requires{typename Compare::is_transparent;}
            iterator first_equiv_greater(const K& key) {return Base_::lower_bound(key);}
            template<typename K> requires requires{typename Compare::is_transparent;}
            const_iterator first_equiv_greater(const K& key) const {return Base_::lower_bound(key);}
            ...
            iterator insert(const key_type& key) {flags_true_(); return Base_::insert(key);}
            iterator insert(key_type&& key) {flags_true_(); return Base_::insert(std::move(key));}
            iterator insert(const_iterator hint, const key_type& key) {flags_true_(); return Base_::insert(hint,key);}
            iterator insert(const_iterator hint, key_type&& key) {flags_true_(); return Base_::insert(hint,std::move(key));}
            template<typename InputIt>
            void insert(InputIt first, InputIt last) {flags_true_(); return Base_::insert(first,last);}
            void insert(std::initializer_list<key_type> il) {flags_true_(); return Base_::insert(il);}
            iterator insert(node_type&& nh) {flags_true_(); return Base_::insert(std::move(nh));}
            iterator insert(const_iterator hint, node_type&& nh) {flags_true_(); return Base_::insert(hint,std::move(nh));}
            ...
            auto insert_equiv(const key_type& key) -> std::conditional_t<signal_suc,std::pair<iterator,bool>,iterator> {
                ...
                if(fg==begin() || key_compare_(*le_candidate,key)){
                    p_has_equal_=true;
                    YUKI_CONDITIONAL_RETURN(signal_suc,YUKI_PROTECT({Base_::insert(fg,key),true}),Base_::insert(fg,key)); // Note that `Base_::insert` does not change the flags.
                }else{
            ...
            auto insert_equiv(key_type&& key) -> std::conditional_t<signal_suc,std::pair<iterator,bool>,iterator> {
                ...
                if(fg==begin() || key_compare_(*le_candidate,key)){
                    p_has_equal_=true;
                    YUKI_CONDITIONAL_RETURN(signal_suc,YUKI_PROTECT({Base_::insert(fg,std::move(key)),true}),Base_::insert(fg,std::move(key)));
                }else{
                    YUKI_CONDITIONAL_RETURN(signal_suc,YUKI_PROTECT({le_candidate,false}),le_candidate);
                }
            }
            ...
            auto insert_equiv(const_iterator hint, const key_type& key) -> std::conditional_t<signal_suc,std::pair<iterator,bool>,iterator> {
                ...
                if(equiv_i==end()){
                    p_has_equal_=true;
                    YUKI_CONDITIONAL_RETURN(signal_suc,YUKI_PROTECT({Base_::insert(hint,key),true}),Base_::insert(hint,key));
                }else{
            ...
            auto insert_equiv(const_iterator hint, key_type&& key) -> std::conditional_t<signal_suc,std::pair<iterator,bool>,iterator> {
                ...
                if(equiv_i==end()){
                    p_has_equal_=true;
                    YUKI_CONDITIONAL_RETURN(signal_suc,YUKI_PROTECT({Base_::insert(hint,std::move(key)),true}),Base_::insert(hint,std::move(key)));
                }else{
                    YUKI_CONDITIONAL_RETURN(signal_suc,YUKI_PROTECT({equiv_i,false}),equiv_i);
                }
            }
            ...
            auto insert_equiv(node_type&& nh) -> std::conditional_t<signal_suc,std::pair<iterator,bool>,iterator> {
                if(nh.empty()){
                    ...
                    if(fg==begin() || key_compare_(*le_candidate,key)){
                        p_has_equal_=true;
                        YUKI_CONDITIONAL_RETURN(signal_suc,YUKI_PROTECT({Base_::insert(fg,std::move(nh)),true}),Base_::insert(fg,std::move(nh)));
                    }else{
                        YUKI_CONDITIONAL_RETURN(signal_suc,YUKI_PROTECT({le_candidate,false}),le_candidate);
                    }
                }
            ...
            auto insert_equiv(const_iterator hint, node_type&& nh) -> std::conditional_t<signal_suc,std::pair<iterator,bool>,iterator> {
                if(nh.empty()){
                    ...
                    if(fe==end()){
                        p_has_equal_=true;
                        YUKI_CONDITIONAL_RETURN(signal_suc,YUKI_PROTECT({Base_::insert(hint,std::move(nh)),true}),Base_::insert(hint,std::move(nh)));
                    }else{
            ...
            auto insert_equal(const key_type& key) -> std::conditional_t<signal_suc,std::pair<iterator,bool>,iterator> {
                ...
                if(lequal==end()){
                    YUKI_CONDITIONAL_RETURN(signal_suc,YUKI_PROTECT({Base_::insert(first_greater(key),key),true}),Base_::insert(first_greater(key),key));
                }else{
            ...
            auto insert_equal(key_type&& key) -> std::conditional_t<signal_suc,std::pair<iterator,bool>,iterator> {
                iterator lequal = last_equal(key);
                if(lequal==end()){
                    YUKI_CONDITIONAL_RETURN(signal_suc,YUKI_PROTECT({Base_::insert(first_greater(key),std::move(key)),true}),Base_::insert(first_greater(key),std::move(key)));
                }else{
                    YUKI_CONDITIONAL_RETURN(signal_suc,YUKI_PROTECT({lequal,false}),lequal);
                }
            }
            ...
            auto insert_equal(node_type&& nh) -> std::conditional_t<signal_suc,std::pair<iterator,bool>,iterator> {
                if(nh.empty()){
                    ...
                    if(lequal==end()){
                        YUKI_CONDITIONAL_RETURN(signal_suc,YUKI_PROTECT({Base_::insert(first_greater(nh.value()),std::move(nh)),true}),Base_::insert(first_greater(nh.value()),std::move(nh)));
                    }else{
                        YUKI_CONDITIONAL_RETURN(signal_suc,YUKI_PROTECT({lequal,false}),lequal);
                    }
                }
            ...
            iterator emplace(Args&&... args) {flags_true_(); return Base_::emplace(std::forward<Args>(args)...);}
            ...
            template<typename... Args>
            iterator emplace_hint(const_iterator hint,Args&&... args) {flags_true_(); return Base_::emplace(hint,std::forward<Args>(args)...);}
